...
import com.madirex.gameserver.dto.login.ListLoginPageDTO;
...
public class LoginController {
    ...
    public ResponseEntity<LoginDTO> findById(@PathVariable String id) {
        ...
        if (login == null) {
            throw new GeneralNotFoundException(id, "No se ha encontrado el login con la id solicitada");
        } else {
    ...
    private void checkLoginData(Login login) {
        if (login.getToken() == null || login.getInstant() == null) {
            throw new GeneralBadRequestException("Token", "El token es obligatorio");
        }
        if (login.getUser() == null) {
            throw new GeneralBadRequestException("User", "El usuario es obligatorio");
        }
    }
}